...
the first node with val = 1, the second node with val = 2, and so on. The graph is represented in the 
    ...
    def cloneGraph(self, node: 'Node') -> 'Node':
        ...
        while q:
            ...
            if currNode not in seen:
                seen.add(currNode)
                if currNode not in dix:
                    dix[currNode] = 1 #= Node(currNode.val)
                for nodes in currNode.neighbors:
                    if nodes not in dix:
                        q.append(nodes)
                        dix[nodes] = 1 #Node(nodes.val)
                    dix[currNode].neighbors.append(dix[nodes])
        return dix[node]